...
public class ScreenRecordingAdapter extends ArrayAdapter<ScreenRecordingAudioSource> {
    ...
    private void initViews() {
        this.mSelectedInternal = getSelected(C0018R$string.screenrecord_device_audio_label);
        this.mSelectedMic = getSelected(C0018R$string.screenrecord_mic_label);
        this.mSelectedMicAndInternal = getSelected(C0018R$string.screenrecord_device_audio_and_mic_label);
        LinearLayout option = getOption(C0018R$string.screenrecord_mic_label, 0);
        this.mMicOption = option;
        option.removeViewAt(1);
        LinearLayout option2 = getOption(C0018R$string.screenrecord_device_audio_and_mic_label, 0);
        this.mMicAndInternalOption = option2;
        option2.removeViewAt(1);
        this.mInternalOption = getOption(C0018R$string.screenrecord_device_audio_label, C0018R$string.screenrecord_device_audio_description);
    }

    private LinearLayout getOption(int i, int i2) {
        LinearLayout linearLayout = (LinearLayout) ((LayoutInflater) getContext().getSystemService("layout_inflater")).inflate(C0014R$layout.screen_record_dialog_audio_source, (ViewGroup) null, false);
        ((TextView) linearLayout.findViewById(C0012R$id.screen_recording_dialog_source_text)).setText(i);
        if (i2 != 0) {
            ((TextView) linearLayout.findViewById(C0012R$id.screen_recording_dialog_source_description)).setText(i2);
        }
        return linearLayout;
    }

    private LinearLayout getSelected(int i) {
        LinearLayout linearLayout = (LinearLayout) LayoutInflater.from(getContext()).inflate(C0014R$layout.screen_record_dialog_audio_source_selected, (ViewGroup) null, false);
        ((TextView) linearLayout.findViewById(C0012R$id.screen_recording_dialog_source_text)).setText(i);
        return linearLayout;
    }
