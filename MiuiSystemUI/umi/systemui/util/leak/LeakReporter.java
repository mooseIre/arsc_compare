...
public class LeakReporter {
    ...
    public void dumpLeak(int i) {
        ...
        try {
            ...
            notificationManager.notify("LeakReporter", 0, NotificationCompat.newBuilder(this.mContext, "leak").setAutoCancel(true).setShowWhen(true).setContentTitle("Memory Leak Detected").setContentText(String.format("SystemUI has detected %d leaked objects. Tap to send", new Object[]{Integer.valueOf(i)})).setSmallIcon(17303602).setContentIntent(PendingIntent.getActivityAsUser(this.mContext, 0, getIntent(file2, file3), 134217728, (Bundle) null, UserHandle.CURRENT)).build());
            return;
        } catch (IOException e) {
