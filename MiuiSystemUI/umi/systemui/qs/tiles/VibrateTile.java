...
public class VibrateTile extends QSTileImpl<QSTile.BooleanState> implements ZenModeController.Callback {
    ...
    public CharSequence getTileLabel() {
        return this.mContext.getString(C0018R$string.quick_settings_vibrate_label);
    }

    /* access modifiers changed from: protected */
    public void handleUpdateState(QSTile.BooleanState booleanState, Object obj) {
        booleanState.value = this.mZenModeController.isVibrateOn();
        booleanState.label = this.mContext.getString(C0018R$string.quick_settings_vibrate_label);
        if (booleanState.value) {
            booleanState.state = 2;
            booleanState.icon = QSTileImpl.ResourceIcon.get(C0010R$drawable.ic_qs_vibrate_on);
        } else {
            booleanState.state = 1;
            booleanState.icon = QSTileImpl.ResourceIcon.get(C0010R$drawable.ic_qs_vibrate_off);
        }
        StringBuilder sb = new StringBuilder();
