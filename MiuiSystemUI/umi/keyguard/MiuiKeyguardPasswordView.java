...
public abstract class MiuiKeyguardPasswordView extends LinearLayout implements EmergencyButton.EmergencyButtonCallback, BackButton.BackButtonCallback {
    ...
    public void onFinishInflate() {
        ...
        EmergencyButton emergencyButton = (EmergencyButton) findViewById(C0014R$id.emergency_call_button);
        this.mEmergencyButton = emergencyButton;
        emergencyButton.setCallback(this);
        BackButton backButton = (BackButton) findViewById(C0014R$id.back_button);
        this.mBackButton = backButton;
        backButton.setCallback(this);
        this.mDeleteButton = (TextView) findViewById(C0014R$id.delete_button);
        if (Build.IS_INTERNATIONAL_BUILD && ((securityMode = ((KeyguardSecurityModel) Dependency.get(KeyguardSecurityModel.class)).getSecurityMode(KeyguardUpdateMonitor.getCurrentUser())) == KeyguardSecurityModel.SecurityMode.Pattern || securityMode == KeyguardSecurityModel.SecurityMode.PIN || securityMode == KeyguardSecurityModel.SecurityMode.Password)) {
            findViewById(C0014R$id.empty_space_for_global).setVisibility(0);
            findViewById(C0014R$id.empty_space).setVisibility(8);
            this.mEmergencyButton.setBackgroundResource(C0012R$drawable.emergency_btn_global);
        }
        this.mKeyguardBouncerMessageView = (KeyguardBouncerMessageView) findViewById(C0014R$id.keyguard_security_bouncer_message);
        MiuiKeyguardFaceUnlockView miuiKeyguardFaceUnlockView = (MiuiKeyguardFaceUnlockView) findViewById(C0014R$id.miui_keyguard_face_unlock_view);
    ...
    public boolean allowUnlock(int i) {
        if (i != 0 && !this.mKeyguardUpdateMonitor.getStrongAuthTracker().hasOwnerUserAuthenticatedSinceBoot()) {
            setSwitchUserWrongMessage(C0020R$string.input_password_after_boot_msg_must_enter_owner_space);
            handleWrongPassword();
            return false;
        } else if (i != KeyguardUpdateMonitor.getCurrentUser() && MiuiKeyguardUtils.isSuperPowerActive(((LinearLayout) this).mContext)) {
            setSwitchUserWrongMessage(C0020R$string.input_password_after_boot_msg_can_not_switch_when_superpower_active);
            handleWrongPassword();
            return false;
        } else if (i != KeyguardUpdateMonitor.getCurrentUser() && MiuiKeyguardUtils.isGreenKidActive(((LinearLayout) this).mContext)) {
            setSwitchUserWrongMessage(C0020R$string.input_password_after_boot_msg_can_not_switch_when_greenkid_active);
            handleWrongPassword();
            return false;
            ...
            setSwitchUserWrongMessage(C0020R$string.input_password_after_boot_msg_can_not_switch_when_calling);
            handleWrongPassword();
            return false;
