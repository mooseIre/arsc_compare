...
public class KeyguardSimPinView extends KeyguardPinBasedInputView {
        ...
        this.mUpdateMonitorCallback = new KeyguardUpdateMonitorCallback() {
            ...
            public void onSimStateChanged(int i, int i2, int i3) {
                ...
                if (PhoneUtils.getPhoneCount() == 2) {
                    ...
                    if (slotIndex == 0) {
                        KeyguardSimPinView.this.mSimImageView.setImageResource(C0012R$drawable.miui_keyguard_unlock_sim_1);
                    } else if (slotIndex == 1) {
                        KeyguardSimPinView.this.mSimImageView.setImageResource(C0012R$drawable.miui_keyguard_unlock_sim_2);
                    }
                } else {
    ...
    private void setLockedSimMessage() {
        ...
        if (activeModemCount < 2) {
            str = resources.getString(C0020R$string.kg_sim_pin_instructions);
        } else {
            ...
            String string = resources.getString(C0020R$string.kg_sim_pin_instructions_multi, subscriptionInfoForSubId != null ? subscriptionInfoForSubId.getDisplayName() : CodeInjection.MD5);
    ...
    private String getPinPasswordErrorMessage(int i, boolean z) {
        ...
        if (i == 0) {
            str = getContext().getString(C0020R$string.kg_password_wrong_pin_code_pukked);
        } else if (i > 0) {
            if (z) {
                i2 = C0018R$plurals.kg_password_default_pin_message;
            } else {
                i2 = C0018R$plurals.kg_password_wrong_pin_code;
            }
            str = getContext().getResources().getQuantityString(i2, i, Integer.valueOf(i));
        } else {
            str = getContext().getString(z ? C0020R$string.kg_sim_pin_instructions : C0020R$string.kg_password_pin_failed);
        }
    ...
    public void verifyPasswordAndUnlock() {
        if (this.mPasswordEntry.getText().length() < 4) {
            this.mSecurityMessageDisplay.setMessage(C0020R$string.kg_invalid_sim_pin_hint);
                        ...
                        public void run() {
                            ...
                            if (pinResult.getType() == 0) {
                                ...
                                if (pinResult.getType() == 1) {
                                    ...
                                    keyguardSimPinView2.mSecurityMessageDisplay.setMessage(keyguardSimPinView2.getContext().getString(C0020R$string.kg_password_pin_failed));
                                }
