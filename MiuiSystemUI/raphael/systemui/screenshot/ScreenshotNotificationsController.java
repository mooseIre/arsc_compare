...
public class ScreenshotNotificationsController {
    ...
    ScreenshotNotificationsController(Context context, WindowManager windowManager) {
        ...
        try {
            this.mResources.getDimensionPixelSize(C0011R$dimen.notification_panel_width);
        } catch (Resources.NotFoundException unused) {
        }
        this.mResources.getDimensionPixelSize(C0011R$dimen.notification_max_height);
    ...
    public void notifyScreenshotError(int i) {
        ...
        Notification.Builder color = new Notification.Builder(this.mContext, NotificationChannels.ALERTS).setTicker(resources.getString(C0020R$string.screenshot_failed_title)).setContentTitle(resources.getString(C0020R$string.screenshot_failed_title)).setContentText(string).setSmallIcon(C0012R$drawable.stat_notify_image_error).setWhen(System.currentTimeMillis()).setVisibility(1).setCategory("err").setAutoCancel(true).setColor(this.mContext.getColor(17170460));
