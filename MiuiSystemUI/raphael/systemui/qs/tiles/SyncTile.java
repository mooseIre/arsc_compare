...
public class SyncTile extends QSTileImpl<QSTile.BooleanState> {
    ...
    public CharSequence getTileLabel() {
        return this.mContext.getString(C0020R$string.quick_settings_sync_label);
    }

    /* access modifiers changed from: protected */
    public void handleUpdateState(QSTile.BooleanState booleanState, Object obj) {
        booleanState.value = isSyncOn();
        booleanState.label = this.mContext.getString(C0020R$string.quick_settings_sync_label);
        if (booleanState.value) {
            booleanState.state = 2;
            booleanState.icon = QSTileImpl.ResourceIcon.get(C0012R$drawable.ic_qs_sync_on);
        } else {
            booleanState.state = 1;
            booleanState.icon = QSTileImpl.ResourceIcon.get(C0012R$drawable.ic_qs_sync_off);
        }
