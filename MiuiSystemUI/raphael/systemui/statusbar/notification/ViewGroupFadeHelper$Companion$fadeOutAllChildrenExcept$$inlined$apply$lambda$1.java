...
final class ViewGroupFadeHelper$Companion$fadeOutAllChildrenExcept$$inlined$apply$lambda$1 implements ValueAnimator.AnimatorUpdateListener {
    ...
    public final void onAnimationUpdate(ValueAnimator valueAnimator) {
        Float f = (Float) this.$root$inlined.getTag(C0014R$id.view_group_fade_helper_previous_value_tag);
        ...
        if (animatedValue != null) {
            ...
            for (View view : this.$viewsToFadeOut$inlined) {
                if (!Intrinsics.areEqual(view.getAlpha(), f)) {
                    view.setTag(C0014R$id.view_group_fade_helper_restore_tag, Float.valueOf(view.getAlpha()));
                }
                view.setAlpha(floatValue);
            }
            this.$root$inlined.setTag(C0014R$id.view_group_fade_helper_previous_value_tag, Float.valueOf(floatValue));
            return;
        }
